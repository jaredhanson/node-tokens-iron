...
module.exports = function(options, keying) {
  ...
  return function iron(claims, /* recipients, */ options, cb) {
    ...
    if (recipients.length > 1) {
      return cb(new Error('Unable to seal iron token to multiple recipients'));
    }
    ...
    if (options.secret) {
      keyed(null, { secret: options.secret, id: options.id });
    } else {
      keying(recipients[0], query, keyed);
    }
  };
};
