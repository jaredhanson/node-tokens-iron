var Iron = require('iron')
  ...
  , ENCRYPTION_ALGORITHMS = require('./constants').ENCRYPTION_ALGORITHMS
  , ENCRYPTION_ALGORITHM_MAP = require('./constants').ENCRYPTION_ALGORITHM_MAP;
...
module.exports = function(options, keying) {
  ...
  return function iron(claims, options, cb) {
    ...
    if (options.encryption.algorithms) {
      supported = options.encryption.algorithms;
      ealg = ENCRYPTION_ALGORITHMS.find(function(e) { return supported.indexOf(e) != -1 });
      if (!ealg) {
        return cb(new Error('Unsupported encryption algorithm: ' + supported[0]));
      }
    }
    ...
    keying(query, function(err, keys) {
      ...
      opts.encryption.algorithm = ENCRYPTION_ALGORITHM_MAP[ealg];
